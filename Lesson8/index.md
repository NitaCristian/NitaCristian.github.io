...
You don’t want a random memory address to be accessed so you initialize a pointer to NULL, a value that can be checked against and one that cannot be a memory address. 
...
If var_name is a variable, &var_name gives the address in memory where its value is placed.
...
You have a pointer to data, containing a valid address. To access that location, you use the dereferencing operator (*). 
If you have a valid pointer pData, use *pDatato access the value stored at the address contained in the pointer.
...
To program an application that is able to optimally consume memory resources, you need to use dynamic memory allocation. 
This enables you to allocate more memory when you need it, and release memory that you have in excess. 
...
For example, the pointer to an integer contains 0x002EFB34. 
The integer itself is 4 bytes long and occupies four places in memory from 0x002EFB34 to 0x002EFB37. 

Incrementing this pointer using operator (++) would not result in the pointer pointing to 0x002EFB35.
...
When it comes to pointers and constants, you have the following combinations: 
...
and give you a pointer to the first element in that array that is identified by the name you assign the array variable.
...
But, C++ does not feature an automatic garbage collector that cleans up the memory your program has allocated but can’t use. 
...
This happens when the programmer did not ensure that his application releases memory allocated dynamically using new with a matching call to delete after the block of memory is no longer required.
...
A valid pointer would be invalid after the call to delete, and should not be used after this point.

## Checking Whether Allocation Request Using new Succeeded

---

There are applications that need to make requests for large chunks of memory. 
...
An exception results in the execution of your application being disrupted. 
...
There is a variant of new called new(nothrow). 
This variant does not throw an exception when allocation requests fail, rather it results in the operator new returning NULL. 
...
It is often important to ensure that the called function cannot change the value of the variable at the caller’s end. 
